...
def video_trans_size(input_mp4, fps):
    ...
    out = iio.get_writer(output_h264, format='ffmpeg', mode='I', fps=fps, codec='libx264', pixelformat='yuv420p', macro_block_size=None)
    while True:
        ret, frame = cap.read()
        if ret:
...
def process_images(base_path):
    ...
    for dir_name in image_dirs:
        ...
        if os.path.exists(color_path) and os.path.exists(gt_path):
            color_images = sorted([os.path.join(color_path, f) for f in os.listdir(color_path) if f.endswith('.png')])
            gt_images = sorted([os.path.join(gt_path, f) for f in os.listdir(gt_path) if f.endswith('.png')])
            ...
            # 生成comparison视频
            comparison_images = sorted([os.path.join(comparison_path, f) for f in os.listdir(comparison_path) if f.endswith('.png')])
            create_video(comparison_images, os.path.join(comparison_path, 'comparison_video.mp4'), 30)
...
# 设置基本路径
base_path = '/media/disk4/zjwang/pixelsplat/outputs/2024-09-22/21-30-46'
